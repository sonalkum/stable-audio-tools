    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[str, tp.Any], negative=False):
        ...
        if len(self.cross_attn_cond_ids) > 0:
            ...
            for key in self.cross_attn_cond_ids:
                # added to check if llm_loss is present
                if len(conditioning_tensors[key]) == 2:
                    cross_attn_in, cross_attn_mask = conditioning_tensors[key]
                elif len(conditioning_tensors[key]) == 3:
                    cross_attn_in, cross_attn_mask, _ = conditioning_tensors[key]
