...
class Attention(nn.Module):
    def __init__(
        ...
        adapter_present = False # added for adapter
    ):
        super().__init__()
        self.dim = dim
        ...
        if dim_context is not None:
            ...
            # if self.adapter_present == True: # added for adapter
            #     self.to_q_adapter_bias = torch.nn.Parameter(torch.zeros(dim))
            #     self.to_q_adapter_scale = torch.nn.Parameter(torch.ones(dim))
            #     self.to_kv_adapter_bias = torch.nn.Parameter(torch.zeros(dim_kv * 2))
            #     self.to_kv_adapter_scale = torch.nn.Parameter(torch.ones(dim_kv * 2))
        else:
            self.to_qkv = nn.Linear(dim, dim * 3, bias=False)
            # if self.adapter_present == True: # added for adapter
            #     self.to_qkv_adapter_bias = torch.nn.Parameter(torch.zeros(dim * 3))
            #     self.to_qkv_adapter_scale = torch.nn.Parameter(torch.ones(dim * 3))

        self.to_out = nn.Linear(dim, dim, bias=False)
        # if self.adapter_present == True: # added for adapter
        #     self.to_out_adapter_bias = torch.nn.Parameter(torch.zeros(dim))
        #     self.to_out_adapter_scale = torch.nn.Parameter(torch.ones(dim))
        ...
        self.dim_kv = dim_kv
        if self.adapter_present == True:
            # added for adapter
            if dim_context is not None:
                self.adapter_wte = nn.Embedding(20, dim_kv)
                self.dim_kv = dim_kv
            else:
                self.adapter_wte = nn.Embedding(20, self.dim)
            # gate for adaption
            self.gating_factor = nn.Parameter(torch.zeros(1, 1, self.num_heads, 1))
    ...
    def forward(
        ...
        # added for adapter
        if self.adapter_present == True:
            if self.dim_kv is None:
                prefix = self.adapter_wte.weight.reshape(1, 20, self.dim)
            else:
                prefix = self.adapter_wte.weight.reshape(1, 20, self.dim_kv)
            prefix = prefix.repeat(x.shape[0], 1, 1)
        ...
        if hasattr(self, 'to_q'):
            ...
            # # added for adapter
            # if self.adapter_present == True:
            #     q = self.to_q_adapter_scale * (q + self.to_q_adapter_bias)
            q = rearrange(q, 'b n (h d) -> b h n d', h = h)

            kv = self.to_kv(kv_input)
            # # added for adapter
            # if self.adapter_present == True:
            #     kv = self.to_kv_adapter_scale * (kv + self.to_kv_adapter_bias)
            k, v = kv.chunk(2, dim=-1)
            k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = kv_h), (k, v))

            # added for adapter
            if self.adapter_present == True:
                prefix_kv = self.to_kv(prefix) #.chunk(2, dim=-1)
                # prefix_kv = self.to_kv_adapter_scale * (prefix_kv + self.to_kv_adapter_bias)
                prefix_k, prefix_v = prefix_kv.chunk(2, dim=-1)
                prefix_k, prefix_v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = kv_h), (prefix_k, prefix_v))
            ...
            qkv = self.to_qkv(x)
            # if self.adapter_present == True:
            #     qkv = self.to_qkv_adapter_scale * (qkv + self.to_qkv_adapter_bias)
            q, k, v = qkv.chunk(3, dim=-1)
            q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), (q, k, v))

            # added for adapter
            if self.adapter_present == True:
                prefix_qkv = self.to_qkv(prefix) #.chunk(3, dim=-1)
                # prefix_qkv = self.to_qkv_adapter_scale * (prefix_qkv + self.to_qkv_adapter_bias)
                _, prefix_k, prefix_v = prefix_qkv.chunk(3, dim=-1)
                prefix_k, prefix_v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), (prefix_k, prefix_v))
        ...
        # Prioritize Flash Attention 2
            ...
            # added for adapter
            if self.adapter_present == True:
                prefix_k, prefix_v = map(lambda t: rearrange(t, 'b h n d -> b n h d').to(torch.float16), (prefix_k, prefix_v))
            # added for adapter
            if self.adapter_present == True:
                out_prefix = flash_attn_func(q, prefix_k, prefix_v, causal = causal)
            # # added for adapter (dont do here and do after gating)
            # if self.adapter_present == True:
            #     out_prefix = rearrange(out_prefix.to(fa_dtype_in), 'b n h d -> b h n d')
        ...
        if self.adapter_present == True:
            out_prefix = self.gating_factor * out_prefix
            out_prefix = rearrange(out_prefix.to(fa_dtype_in), 'b n h d -> b h n d')
            out = out + out_prefix
        ...
        out = self.to_out(out)
        # if self.adapter_present == True:
        #     out = self.to_out_adapter_scale * (out + self.to_out_adapter_bias)
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = Attention(
            ...
            adapter_present = adapter_present, # added for adapter,
            **attn_kwargs
        )
