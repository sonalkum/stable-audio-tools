...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        # Added for DPO
        self.diffusion_dpo = copy.deepcopy(self.diffusion.model)
        self.diffusion_dpo.model.adapter_present = False # added for adapter
        self.diffusion_dpo.requires_grad_(False)
    ...
    def filter_params(self, model):
        for name, param in model.named_parameters():
            if "adapter" in name or "gating_factor" in name:
    ...
    def configure_optimizers(self):
        ...
        #     print(key)
        # print(self.diffusion.parameters().keys())
        # print('----------------------------------------')
        diffusion_opt_config = self.optimizer_configs['diffusion']

        # added for adapter
        if diffusion_opt_config['adapter_present'] == True:
            print('Only fine-tuning adapter!')
            self.filter_params(self.diffusion.model)
        opt_diff = create_optimizer_from_config(diffusion_opt_config['optimizer'], [p for p in self.diffusion.model.parameters() if p.requires_grad]) # changed to diffusion.model.parameters() from diffusion.parameters() for DPO
    ...
    def training_step(self, batch, batch_idx):
        reals, reals_np, metadata = batch

        # # dummy
        # reals_pref = copy.deepcopy(reals)
        ...
        diffusion_input = reals #torch.Size([8, 2, 2097152])
        diffusion_input_dpo = reals_np # added for DPO

        # Concatenate across batch for easier further processing
        diffusion_input = torch.cat((diffusion_input, diffusion_input_dpo), dim=0) # added for DPO


        if not self.pre_encoded:
            loss_info["audio_reals"] = diffusion_input
            loss_info_dpo["audio_reals"] = diffusion_input_dpo # added for DPO

        p.tick("setup")

        with torch.cuda.amp.autocast():
            conditioning = self.diffusion.conditioner(metadata, self.device)
            # repeat everything for DPO
            conditioning['prompt'] = (conditioning['prompt'][0].repeat(2,1,1), conditioning['prompt'][1].repeat(2,1))
            conditioning['seconds_start'] = [conditioning['seconds_start'][0].repeat(2,1,1), conditioning['seconds_start'][1].repeat(2,1)]
            conditioning['seconds_total'] = [conditioning['seconds_total'][0].repeat(2,1,1), conditioning['seconds_total'][1].repeat(2,1)]
        ...
        if use_padding_mask:
            # probably not being used so ignoring to repeat this
        ...
        if self.diffusion.pretransform is not None:
            ...
            if not self.pre_encoded:
                with torch.cuda.amp.autocast() and torch.set_grad_enabled(self.diffusion.pretransform.enable_grad):
                    ...
                    # diffusion_input is already concatenated so pass once
                    diffusion_input = self.diffusion.pretransform.encode(diffusion_input)
                    # diffusion_input_dpo = self.diffusion.pretransform.encode(diffusion_input_dpo) # added for DPO
                    p.tick("pretransform")
                ...
                # the code goes here, only only focusing on this            
                # Apply scale to pre-encoded latents if needed, as the pretransform encode function will not be run
                if hasattr(self.diffusion.pretransform, "scale") and self.diffusion.pretransform.scale != 1.0:
                    diffusion_input = diffusion_input / self.diffusion.pretransform.scale
                    # diffusion_input_dpo = diffusion_input_dpo / self.diffusion.pretransform.scale # added for DPO

        if self.timestep_sampler == "uniform":
            # Draw uniformly distributed continuous timesteps
            t = self.rng.draw(reals.shape[0])[:, 0].to(self.device).repeat(2) # added for DPO repeat(2) we want same timestep for both parts
        elif self.timestep_sampler == "logit_normal":
            t = torch.sigmoid(torch.randn(reals.shape[0], device=self.device)).repeat(2) # added for DPO repeat(2) we want same timestep for both parts
        ...
        alphas = alphas[:, None, None]
        sigmas = sigmas[:, None, None]
        noise = torch.randn_like(diffusion_input.chunk(2)[0]).repeat(2,1,1) # added for DPO .chunk(2)[0]).repeat(2,1,1) we want same noise for both parts
        noised_inputs = diffusion_input * alphas + noise * sigmas
        ...
        with torch.cuda.amp.autocast():
            ...
            # added for DPO
            with torch.no_grad():
                output_dpo_conditioning_inputs = self.diffusion.get_conditioning_inputs(conditioning)
                # output_dpo_conditioning_inputs['cross_attn_cond'] = output_dpo_conditioning_inputs['cross_attn_cond'].repeat(2,1,1)
                # output_dpo_conditioning_inputs['cross_attn_mask'] = output_dpo_conditioning_inputs['cross_attn_mask'].repeat(2,1)
                # output_dpo_conditioning_inputs['global_cond'] = output_dpo_conditioning_inputs['global_cond'].repeat(2,1)
                output_dpo = self.diffusion_dpo(noised_inputs, t, **output_dpo_conditioning_inputs, cfg_dropout_prob = self.cfg_dropout_prob, **extra_args)
                p.tick("diffusion_dpo")

                loss_info_dpo.update({
                "output": output_dpo,
                "targets": targets,
                "padding_mask": padding_masks if use_padding_mask else None,
                })

                loss_dpo, losses_dpo = self.losses(loss_info_dpo)

                loss_w_dpo, loss_l_dpo = loss_dpo.chunk(2)

                dpo_diff = loss_w_dpo - loss_l_dpo
            ...
            scale_term = -0.5 * 2000
            inside_term = scale_term * (diff - dpo_diff)
            final_loss = -1 * F.logsigmoid(inside_term).mean()
        ...
        log_dict = {
            'train/loss': final_loss.detach(), # change for DPO --> originally loss.detach()
            'train/std_data': diffusion_input.std(),
            'train/lr': self.trainer.optimizers[0].param_groups[0]['lr']
        }

        log_dict[f"train/dpo"] = final_loss.detach()

        # for loss_name, loss_value in losses.items():
        #     log_dict[f"train/{loss_name}"] = loss_value[0].detach()
