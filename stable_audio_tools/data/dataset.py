...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.csv_dataset = pd.read_csv(configs[0].path)

        self.filenames = list(self.csv_dataset['preferred'])
        self.filenames_np = list(self.csv_dataset['not_preferred'])
        self.captions = list(self.csv_dataset['captions'])
    ...
    def __getitem__(self, idx):
        ...
        audio_np_filename = self.filenames_np[idx]
        ...
        try:
            ...
            # for not preferred audio
            audio_np = self.load_file(audio_np_filename)

            audio_np, t_start, t_end, seconds_start, seconds_total, padding_mask = self.pad_crop(audio_np) # check how to not randomize this

            # Run augmentations on this sample (including random crop)
            if self.augs is not None:
                audio_np = self.augs(audio_np)

            audio_np = audio_np.clamp(-1, 1)

            # Encode the file to assist in prediction
            if self.encoding is not None:
                audio_np = self.encoding(audio_np)
            ...
            info["prompt"]= self.captions[idx][len("Audio caption: "):]
